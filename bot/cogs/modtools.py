...
Commands (all require appropriate permissions):
  /slowmode      - Set channel slowmode
  /lock          - Lock a channel
  /unlock        - Unlock a channel
  /clear         - Purge N messages
  /kick          - Kick a member
  /ban           - Ban a member
  /unban         - Unban by user ID
  /mute          - Timeout a user (Discord timeout)
  /unmute        - Remove timeout
  /nickname      - Change a member's nickname
  /addrole       - Add a role to a user
  /removerole    - Remove a role from a user
  /roleinfo      - Info about a role
  /rolemembers   - List members with a role
  /channelinfo   - Info about a channel
  /voicemove     - Move all members from one VC to another
  /voicekick     - Disconnect a user from voice
  /massrole      - Add/remove a role to all members (admin)
  /nuke          - Delete and recreate a channel (admin)
  /warn_user     - Add a warning to a user's record
  /warnings      - View a user's warning record
  /clearwarnings - Clear a user's warnings (admin)
  /muterole      - Auto-create and apply a mute role
  /note          - Add a mod note to a user
...
def _mod_log_row(action: str, target: discord.Member | str, mod: discord.Member, reason: str) -> str:
    name = target.display_name if isinstance(target, discord.Member) else str(target)
    return f"**{action}** · {name} · by {mod.display_name} · _{reason or 'No reason'}_"
...
class ModTools(commands.Cog):
    ...
    async def lock(self, ctx, channel: discord.TextChannel = None, *, reason: str = "No reason given"):
        ...
        e = embed("🔒  Channel Locked", f"{ch.mention} is now locked.\n\n_{reason}_", C.WARN,
    ...
    @commands.hybrid_command(name="clear", description="Delete N messages from this channel.")
    ...
    async def clear(self, ctx, amount: int, member: discord.Member = None):
        ...
        await ctx.send(embed=success("Cleared", f"Deleted **{len(deleted)}** message(s){f' from {member.mention}' if member else ''}."), ephemeral=True)

    @commands.hybrid_command(name="nuke", description="Delete and recreate a channel (admin only).")
    ...
    async def nuke(self, ctx, *, reason: str = "Nuked"):
        ...
        await ch.delete()
        await new.send(embed=embed("💥  Nuked", f"This channel was nuked by {ctx.author.mention}.\n_{reason}_", C.LOSE,
                                   footer=f"Nuked by {ctx.author.display_name}"))

    @commands.hybrid_command(name="channelinfo", description="Info about a channel.")
    ...
    @commands.hybrid_command(name="mute", description="Timeout a user for a set number of minutes.")
    ...
    async def mute(self, ctx, member: discord.Member, minutes: int = 10, *, reason: str = "No reason given"):
        ...
        e = embed("🔇  Muted", f"**{member.display_name}** timed out for **{minutes}m**.\nReason: _{reason}_", C.WARN)
        e.set_footer(text=f"By {ctx.author.display_name}")
        await ctx.send(embed=e)

    @commands.hybrid_command(name="unmute", description="Remove a user's timeout.")
    @commands.has_permissions(moderate_members=True)
    ...
    @commands.hybrid_command(name="nickname", description="Change a member's server nickname.")
    ...
    @commands.hybrid_command(name="roleinfo", description="Info about a role.")
    ...
    @commands.hybrid_command(name="rolemembers", description="List members who have a specific role.")
    async def rolemembers(self, ctx, role: discord.Role):
        ...
        e = embed(f"🎭  {role.name} — {len(role.members)} member(s)",
                  names + extra, C.ADMIN)
        await ctx.send(embed=e)

    @commands.hybrid_command(name="massrole", description="Add or remove a role from all members (admin).")
    @commands.has_permissions(administrator=True)
    async def massrole(self, ctx, action: str, role: discord.Role):
        ...
        for member in ctx.guild.members:
            ...
            try:
                ...
                pass
        await ctx.send(embed=success(f"Mass Role — {action.capitalize()}d",
                                     f"{action.capitalize()}d {role.mention} for **{count}** member(s)."))

    # ══ VOICE MANAGEMENT ══════════════════════════════════════════════════════

    @commands.hybrid_command(name="voicemove", description="Move all members from one VC to another.")
    ...
    @commands.hybrid_command(name="voicekick", description="Disconnect a user from voice.")
    @commands.has_permissions(move_members=True)
    ...
    @commands.hybrid_command(name="warn_user", description="Add a warning to a user's record.")
    @commands.has_permissions(manage_messages=True)
    async def warn_user(self, ctx, member: discord.Member, *, reason: str):
        ...
        try:
            await member.send(embed=warn("Warning Received",
                                         f"You received a warning in **{ctx.guild.name}**.\nReason: _{reason}_\nYou now have **{count}** warning(s)."))
        except Exception:
            pass
        e = embed(f"⚠️  Warning #{count} — {member.display_name}",
                  f"Reason: _{reason}_\nTotal warnings: **{count}**", C.WARN,
    ...
    @commands.hybrid_command(name="clearwarnings", description="Clear all warnings for a user (admin only).")
    @commands.has_permissions(administrator=True)
    ...
    @commands.hybrid_command(name="note", description="Add a mod note to a user (invisible to them).")
    @commands.has_permissions(manage_messages=True)
    async def note(self, ctx, member: discord.Member, *, note_text: str):
        ...
        await ctx.send(embed=success("Note Added 📝",
                                     f"Note added to **{member.display_name}** (#{len(notes[uid])})."),
                       ephemeral=True)
    ...
    async def notes(self, ctx, member: discord.Member):
        ...
        if not notes:
            return await ctx.send(embed=embed("📝  No Notes", f"No notes for **{member.display_name}**.", C.NEUTRAL), ephemeral=True)
        lines = [f"**#{i}** <t:{n['at']}:d> — _{n['text']}_ (by {n['by']})" for i, n in enumerate(notes[-10:], 1)]
    ...
    async def cog_command_error(self, ctx, error_obj):
        if isinstance(error_obj, commands.MissingPermissions):
            await ctx.send(embed=error("Permission Denied",
                                       f"You're missing: `{', '.join(error_obj.missing_permissions)}`"))
        elif isinstance(error_obj, commands.BotMissingPermissions):
            await ctx.send(embed=error("Bot Missing Permissions",
                                       f"I need: `{', '.join(error_obj.missing_permissions)}`"))
        else:
            raise error_obj
